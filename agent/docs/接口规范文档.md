...
本文档定义 Agent 与 Daemon 之间的通信协议，以及 Agent 自身提供的 HTTP API 规范。

## 2. 心跳数据格式

### 2.1 心跳数据结构（JSON）

Agent 向 Daemon 发送的心跳数据采用 JSON 格式，结构如下：
...
## 3. 心跳 API 规范（Agent → Daemon）

### 3.1 心跳上报端点

**端点**: `POST http://daemon-host:port/heartbeat`

**说明**: Agent 定期向 Daemon 发送心跳数据，用于 Daemon 监控 Agent 运行状态。
...
Agent 提供以下 HTTP API 端点，用于健康检查、配置重载和指标查询。
...
**文档完成时间**：2025-01-XX  
**下一步**：整合所有设计文档，编写完整的 Agent 架构设计文档（步骤 4）
